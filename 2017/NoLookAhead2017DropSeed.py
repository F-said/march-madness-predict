...
# Performance was better without seeds
...
X_train_2017 = X_train[X_train["Season"] < 2017].drop(labels="Season", axis=1)
X_train_2017_selected = X_train_selected[X_train["Season"] < 2017]
y_train_2017 = y_train[X_train["Season"] < 2017]

X_test_2017 = X_train[X_train["Season"] == 2017].drop(labels="Season", axis=1)
X_test_2017_selected = X_train_selected[X_train["Season"] == 2017]
y_test_2017 = y_train[X_train["Season"] == 2017]

submission2017 = pd.read_csv("/Users/farukhsaidmuratov/PycharmProjects/MarchMadness/2017/X_test_seedordinal_2017.csv").\
    drop(labels="Team1", axis=1).drop(labels="Team2", axis=1).drop(labels="Season", axis=1).drop(labels="SeedDiff", axis=1)
submission2017_selected = pd.read_csv("/Users/farukhsaidmuratov/PycharmProjects/MarchMadness/2017/X_test_seedordinal_2017.csv")\
...
sub_file.to_csv(path_or_buf="submission_seedordinal_noseed2017_nn.csv", index=False)

sub_file = sub_file.drop(labels="Pred", axis=1)
sub_file.insert(1, "Pred", y_pred_2017)
sub_file.to_csv(path_or_buf="submission_seedordinal_noseed2017_gb.csv", index=False)

'''
Accuracies
Accuracy GB no seed:  0.731343283582
