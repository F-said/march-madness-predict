...
# pull training data 
X_train = pd.read_csv("\\form_data\X_train_seedordinal.csv").\
    drop(labels="Team1", axis=1).drop(labels="Team2", axis=1)
y_train = pd.read_csv("\\form_data\y_train_seedordinal.csv")
...
real_test = pd.read_csv("\\form_data\X_test_seedordinal.csv").drop(labels="Team1", axis=1).drop(labels="Team2", axis=1).drop(labels="Season", axis=1)
sub_file = pd.read_csv("\\unform_data\MSampleSubmissionStage1.csv").drop(labels="Pred", axis=1)
